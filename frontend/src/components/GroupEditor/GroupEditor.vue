<template>
  <dialog ref="dialogEl" class="group-editor" @close="emit('update:open', false)">
    <form class="group-editor__form" @submit.prevent="handleSubmit">
      <h2 class="group-editor__title">{{ mode === 'create' ? 'Create Group' : 'Edit Group' }}</h2>
      ...
      <div class="group-editor__field">
        ...
        <div class="group-editor__swatches" role="radiogroup" aria-label="Group color">
          <button
            ...
            :class="{ 'is-selected': isSameColor(color, swatch.value) }"
      ...
      <p v-if="errorMessage" class="group-editor__error" role="alert">{{ errorMessage }}</p>
...
function handleSubmit() {
  ...
  emit('submit', { name: name.value.trim(), color: color.value, enabled: enabled.value })
}
</script>
