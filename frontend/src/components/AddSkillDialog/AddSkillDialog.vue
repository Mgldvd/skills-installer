    ...
    <form class="add-skill-dialog__form" @submit.prevent="handleSubmit">
      ...
      <div class="add-skill-dialog__body">
        ...
        <div v-else-if="packPreview" class="add-skill-dialog__preview add-skill-dialog__wide">
          <p>Detected Pack: <strong>{{ packPreview.skills.length }} Skills</strong></p>
          <div class="add-skill-dialog__pack-skills">
            <span v-for="skill in packPreview.skills" :key="skill.name">{{ skill.name }}</span>
          </div>
          <p class="add-skill-dialog__catalog-note">This imports catalog entries only. Nothing is installed into the current project.</p>
        </div>
        ...
        <div v-if="isPackUrl" class="add-skill-dialog__field">
          ...
          <div class="add-skill-dialog__colors" role="radiogroup" aria-label="Pack color">
            <button v-for="color in packColors" :key="color" type="button" :class="{ 'is-active': packColor === color }" :style="{ backgroundColor: color }" :aria-label="`Use color ${color}`" :aria-pressed="packColor === color" @click="packColor = color" />
          </div>
        </div>
...
const packColors = ['#F43F75', '#F05252', '#F97316', '#F59E0B', '#22C55E', '#14B8A6', '#06B6D4', '#3B82F6', '#6366F1', '#A855F7']
...
function handleUrlInput() {
  ...
  debounceHandle = setTimeout(async () => {
    ...
    try {
      ...
      const detected = pack ? await backend.previewPackUrl(url.value.trim()) : await backend.previewSkillUrl(url.value.trim())
...
const canSubmit = computed(() => Boolean((preview.value || packPreview.value) && displayName.value.trim() && groupId.value && !urlError.value && (!preview.value || !duplicateSkill.value)))
...
function handleSubmit() {
  ...
  if (packPreview.value) {
    emit('importPack', { url: url.value.trim(), packName: displayName.value.trim(), color: packColor.value, groupId: groupId.value })
    return
  }
  emit('submit', {
    url: url.value.trim(),
