<template>
  <dialog ref="dialogEl" class="skills-dialog" @close="emit('update:open',false)">
    <section class="skills-dialog__panel">
      ...
      <div class="skills-dialog__source"><h3>Local Skill Source</h3><p>This folder is a catalog of installable Skills, not an installation destination.</p><label><span>Source folder</span><input v-model="source" type="text" placeholder="~/.control/skill" /></label><div><button type="button" @click="saveSource">Change Folder</button><button type="button" @click="emit('refresh')">Refresh</button></div></div>
    </section>
  </dialog>
</template>
...
const dialogEl=ref<HTMLDialogElement|null>(null),source=ref('')
watch(()=>props.open,open=>{if(open)source.value=props.localSourcePath??'~/.control/skill'})
useNativeDialog(dialogEl,()=>props.open)
function close(){emit('update:open',false)}
function saveSource(){if(source.value.trim())emit('updateLocalSource',source.value.trim())}
</script>
