<template>
  <dialog ref="dialogEl" class="delete-group-dialog" @close="emit('update:open', false)">
    <div class="delete-group-dialog__body">
      ...
      <p v-if="skillCount === 0" class="delete-group-dialog__message">This group is empty and can be deleted safely.</p>
      <template v-else>
        <p class="delete-group-dialog__message">
          This group still has {{ skillCount }} skill{{ skillCount === 1 ? '' : 's' }}. Choose what happens to them —
          skills are never deleted along with their group.
        </p>
...
function handleConfirm() {
  ...
  if (props.skillCount === 0) {
    ...
    emit('confirm', { kind: 'move-to', targetGroupId: targetGroupId.value })
  }
  emit('update:open', false)
}
</script>
