...
describe("InstallProgressPanel", () => {
  ...
  it("shows Cancel while installing and a dismiss control once finished", async () => {
    const installing = mount(InstallProgressPanel, {
      props: { installation: baseInstallation({ isInstalling: true }) },
    });
    ...
    const finished = mount(InstallProgressPanel, {
      props: {
        installation: baseInstallation({
          result: { requested: 1, installed: 1, alreadyInstalled: 0, failed: 0, cancelled: false, perSkill: [] },
        }),
      },
    });
